...
| `dispatch-popup.js` | Antenne corner popup (fleet news from dispatch.neorgon.com), desktop only, once per story, collapses to a satellite dock on dismiss. Fetches after idle. **On localhost it logs one `ERR_CONNECTION_REFUSED`** for `localhost:8873/data/posts.json`, which is the news site not running; the fetch is caught, so this is Chrome's network log rather than an unhandled rejection |
