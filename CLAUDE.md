...
| `previews.js` | Opt-in card previews: delayed hover plus explicit keyboard/touch controls. Delegated listeners support catalog cards and shelf echoes; only one preview is active. Escape/close restores focus, leaving the control with Tab closes it, and preference changes dismiss it immediately. Reduced motion decodes a GIF off-DOM and paints one canvas still, then releases the image. `_neoPreviewsEnabled` is read from settings, never overwritten. Settings announces changes through `neorgon:previews-change`. |
...
| `dispatch-popup.js` | Native Antenne bulletin (styles in `css/dispatch-popup.css`). Desktop fetches after idle; mobile offers a dock and fetches only on activation, validates and renders the newest three stories, and reuses local catalog icons with launch/feature/fix/note badges. Each story names its recognized linked tools with their local icons, deduplicates destinations, and puts a recognized primary site first. Dismissal stores a versioned list of story IDs and dates in `neorgon-dispatch-seen`, so another story on the same day reopens the bulletin. Legacy date-only values reopen once. The dock reopens it; close/Escape returns focus. Loading, empty, error, and retry states share the same native view; outside taps dismiss on mobile. A 10-second timeout prevents a stalled feed from waiting forever. No iframe or remote favicon fetch. The localhost feed is `localhost:8873/data/posts.json`; an unavailable feed is caught and leaves the catalog usable. |
