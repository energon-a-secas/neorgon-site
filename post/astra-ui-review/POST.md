...
There is a cost to removing the iframe: the hub now owns a small news renderer. If Antenne adds a new story kind, this renderer needs to learn it. It currently skips unsupported kinds. A story about several tools gets the first catalog icon it can match from its links, which is useful shorthand but incomplete. Unknown tools get the satellite.

I have no measured discovery-speed improvement to report. No controlled comparison with another model, either. Production deployment and a live CORS check remain separate work. This session demonstrates a bounded interface change with inspectable results; it does not settle a model ranking.

The next judgment is visual: open the page, scan the catalog, and see whether Antenne belongs there now.
